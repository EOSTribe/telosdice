...
void bocai2222::reveal(const uint64_t &id)
{
    ...
    {
        issue_token(bet.player, bet.amount, "mining! telosdice.vip");
        unlock(bet.amount);
    }
...
void bocai2222::onTransfer(account_name from,
                              account_name to,
                              ...
                              string memo)
{
    ...
    action(permission_level{_self, N(active)},
           ...
           N(transfer),
           std::make_tuple(_self, N(bocai1111), compute_pool_reward(_bet), std::string("Dividing pool")))
        .send();
    send_defer_action(permission_level{_self, N(active)},
                      _self,
                      N(reveal1),
...
void bocai2222::addtoken(account_name contract, asset quantity)
{
    require_auth(_self);
