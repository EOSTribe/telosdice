...
class bocai2222 : public contract
{
  public:
    bocai2222(account_name self)
        : contract(self),
          _bets(_self, _self),
          _users(_self, _self),
  ...
  private:
    ...
    string referrer_memo(const st_bet &bet)
    {
        ...
        memo.append(player);
        memo.append(" referral reward! tlosdice.vip");
        return memo;
    }
    ...
    string winner_memo(const st_bet &bet)
    {
        ...
        string player = name{bet.player}.to_string();
        memo.append(player);
        memo.append(" winner! tlosdice.vip");
        return memo;
    }
    ...
    {
        if ((quantity.contract == N(eosio.token)) && (quantity.symbol == TLOS_SYMBOL))
        {
            return true;
        }
        return false;
    ...
    {
        st_fund_pool fund_pool{.locked = asset(0, TLOS_SYMBOL)};
    ...
    void vipcheck(account_name from, asset quantity)
    {
        ...
        asset checkout = asset(0, TLOS_SYMBOL);
        if (amount < 1000)
        {
            return;
        }
        ...
        {
            checkout = asset(quantity.amount * 1 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 10000)
        {
            checkout = asset(quantity.amount * 2 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 50000)
        {
            checkout = asset(quantity.amount * 3 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 100000)
        {
            checkout = asset(quantity.amount * 4 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 500000)
        {
            checkout = asset(quantity.amount * 5 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 1000000)
        {
            checkout = asset(quantity.amount * 7 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 5000000)
        {
            checkout = asset(quantity.amount * 9 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 10000000)
        {
            checkout = asset(quantity.amount * 11 / 10000, TLOS_SYMBOL);
        }
        else if (amount < 50000000)
        {
            checkout = asset(quantity.amount * 13 / 10000, TLOS_SYMBOL);
        }
        else
        {
            checkout = asset(quantity.amount * 15 / 10000, TLOS_SYMBOL);
        }
        action(permission_level{_self, N(active)},
               N(eosio.token),
               N(transfer),
